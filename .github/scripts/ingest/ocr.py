...
3. **竖排版面（字从上到下、列从右到左）必须按正确阅读顺序转录：最右侧的列
   是第一行，依次向左**；每列转录为一行
4. **输出统一使用简体中文**：印刷体若为繁体/异体字，转写为对应简体
   （这是字形转换，不是改写歌词）
5. 保留原有换行与分行；若版面能明显区分「歌词区」与「制作信息区」，可在各区前加一行标记
   `[LYRICS]` 或 `[CREDITS]`（仅当你有把握时，不确定就直接顺序转录）
6. 忽略纯装饰性符号/水印/页码
7. 若图片中没有任何文字，只输出一行：[NO_TEXT]
8. 不要输出任何解释或额外说明，只输出转录正文"""
