...
- [现状] -> Phase B 的 `finalize` 只写 `track.lrc` 与 `track.klrc`。 -> `track.vocals` 会留在 review draft 而不会进入成品。
- [数据结构] -> 声部草稿包含 `lrc`、`klrc`、`lines`、`timing_locked` 和展示名称。 -> 只消费 `timing_locked` 的非空时间行，声部名称不得写进成品歌词。
- [兼容性] -> 单声部必须保留原字符串。 -> 无可合并声部时直接返回原对象内容。
- [验证] -> 运行新增多重唱产物测试及既有 timing-lock、中文简体测试。 -> 6 项通过。
