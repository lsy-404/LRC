...
import {
  activeIndexAt, clampWordTime, expandTimedTokens, mergeTimedRows, mergeTimedToken, moveTimedSelection, parseLrc, parseKaraokeRows, reconcileTimedRows, reconcileWordCharacters, serializeTimedLyrics, splitTimedRow, splitTimedToken, splitRowAtTokenBoundary, textToLines, linesToText, timedLeadSpanPx, timedTokenSpanMs, timedTokenSpanPx, utf16ToCodePointIndex, isTrackEdited, isLowCoverage, msToTimestamp,
} from './lrcDraft.js';
...
.eb-time-token { --token-progress: 0%; position: relative; box-sizing: border-box; display: flex; flex: 0 0 auto; min-width: 0; flex-direction: column; justify-content: space-between; min-height: 3.2rem; white-space: nowrap; border-left: 1px solid color-mix(in srgb, var(--eb-accent) 45%, transparent); }
