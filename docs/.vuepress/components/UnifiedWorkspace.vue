    ...
    <div class="uw-shell">
      ...
      <main class="uw-editor">
        <template v-if="selected">
          <div class="uw-tab"><strong>{{ selected.name }}</strong><span>{{ selected.kind === 'meta' ? '整专信息' : selected.remote ? '工作区歌词' : '待上传文件' }}</span></div>
          <MonacoLrcEditor v-model="selected.content" :language="selected.language" :theme="theme" :aria-label="`${selected.name} 编辑器`" />
          <footer class="uw-actions"><button class="uw-btn" type="button" :disabled="!selected.remote || saving" @click="save">{{ saving ? '保存中…' : '保存工作草稿' }}</button><span class="uw-dim">{{ message }}</span></footer>
        </template>
      </main>
    </div>
  </section>
...
async function save() {
  const remote = selected.value?.remote; if (!remote) return; saving.value = true; message.value = '';
  try {
