...
本项目使用两种主要文件格式，它们都可以用**任何文本编辑器**打开和编辑：
...
| 文件类型 | 位置 | 说明 |
|---------|------|-----|
| **info.toml** | `/res/[专辑名]/` | 专辑元数据（专辑名、艺术家、发布日期等） |
| **meta.toml** | `/res/[专辑名]/[曲目名]/` | 单首歌曲的元数据（歌手、作词、作曲等） |
...
#### 元数据校验
- **必填字段检查**：确保 info.toml 和 meta.toml 包含必需的字段
- **字段格式校验**：日期、链接、标签等字段格式正确
...
3. **一个 PR 专注一个任务**：不要在一个 PR 中混合多个不相关的修改
